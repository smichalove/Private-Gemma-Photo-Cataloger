...
OUTPUT_JSON: str = os.environ.get("OUTPUT_DATABASE_PATH", os.path.join(PROJECT_DIR, "photo_descriptions.json"))
DB_PATH: str = os.environ.get("OUTPUT_DATABASE_SQLITE", os.path.join(os.path.dirname(os.path.abspath(__file__)), "photo_catalog.db"))
SUBMITTED_CACHE: str = os.environ.get("SUBMITTED_CACHE_PATH", os.path.join(PROJECT_DIR, "submitted_photos_cache.txt"))
            ...
            try:
                ...
                if "Bad Photoshop IRB resource" in err_msg:
                    try:
                        ...
                        pass
                elif "Truncated InteropIFD directory" in err_msg:
                    try:
                        fallback_cmd: List[str] = cmd.copy()
                        fallback_cmd.insert(4, "-InteropIFD:All=")
                        subprocess.run(fallback_cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, timeout=15.0)
                        if verify_embedded_metadata(file_path, summary_text):
                            logger.info(f"EXIF embedded and verified successfully (InteropIFD rebuild fallback): {file_path}")
                            return
                    except Exception:
                        pass
...
def extract_json_payload(raw_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if sub_m:
            parsed["primary_subject"] = sub_m.group(1).replace('\\"', '"').replace('\\\\', '\\').replace('\\n', '\n').replace('\\t', '\t')
            
        # 2. Extract environment
        env_m = re.search(r'"?environment"?\s*:\s*"(.*?)"(?=\s*(?:,|\s*\}|\s*"))', cleaned, re.DOTALL | re.IGNORECASE)
        if env_m:
            parsed["environment"] = env_m.group(1).replace('\\"', '"').replace('\\\\', '\\').replace('\\n', '\n').replace('\\t', '\t')
...
def process_batches(
    ...
    def worker(server: wsl_client.VLMServerConnection) -> None:
        ...
        # If this is the local server and it is not yet alive, boot it asynchronously in this thread (only if in legacy VLM mode)
        if server.name == "Local RTX 5080" and not server.is_alive():
            if server.mode == "vlm":
                ...
                logger.error("[Local RTX 5080] Local Ollama service is offline! Please start Ollama (e.g. run 'ollama serve' or open the Ollama app) and pull the model before running. Worker exiting.")
                return
    ...
    for srv in active_servers:
        t = threading.Thread(target=worker, args=(srv,), name=f"Worker-{srv.name}", daemon=True)
        t.start()
        threads.append(t)
        
    # Wait for all workers to complete while keeping the main thread responsive to signals (e.g. Ctrl+C on Windows)
    try:
        while any(t.is_alive() for t in threads):
            time.sleep(0.1)
    except KeyboardInterrupt:
        logger.warning("Main thread interrupted. Aborting worker threads.")
        raise
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="gemma4:12b",
        help="VLM model name to request and run health checks for (e.g. gemma4:12b, gemma4:31b)."
    )
    ...
    # Probe and set up VLM server connections dynamically using FabricManager
    active_servers: List[wsl_client.VLMServerConnection] = []
    
    try:
        logger.info("[FABRIC] Initializing Compute Fabric registry for VLM processing...")
        fabric_mgr = FabricManager()
        logger.info("[FABRIC] Discovering active fabric servers in background (waiting up to 5.0s)...")
        fabric_mgr.wait_for_discovery(timeout=5.0)
        
        # Print a diagnostic log of all registered nodes and their current health status
        logger.info(f"[FABRIC] Total registry nodes: {len(fabric_mgr.nodes)} | Active/Online: {len(fabric_mgr.active_pool)} | Watchlist/Offline: {len(fabric_mgr.watchlist)}")
        for h, n in sorted(fabric_mgr.nodes.items()):
            logger.info(f"[FABRIC]   - Node: {n.hostname:15s} | IP: {str(n.resolved_ip or 'None'):15s} | Online: {str(n.is_online):5s} | Services: {list(n.services.keys())} | Models: {n.supported_models}")

        # Retrieve all online nodes capable of running the requested model with either 'ollama' or 'vlm' services
        nodes_ollama = fabric_mgr.get_nodes_for_workload(args.model, "ollama")
        nodes_vlm = fabric_mgr.get_nodes_for_workload(args.model, "vlm")
        
        # Merge ensuring unique nodes
        seen_hosts = set()
        nodes = []
        for node in nodes_ollama + nodes_vlm:
            if node.hostname not in seen_hosts:
                seen_hosts.add(node.hostname)
                nodes.append(node)
        
        # Fall back to any active ollama/vlm nodes if none specifically registered the model support
        if not nodes:
            logger.info(f"[FABRIC] No nodes found explicitly supporting '{args.model}'. Querying any active 'ollama' or 'vlm' nodes...")
            nodes = [node for node in fabric_mgr.get_active_nodes() if "ollama" in node.services or "vlm" in node.services]
            
        import socket
        for node in nodes:
            # Determine which service mode and port to connect with
            service_mode = "ollama" if "ollama" in node.services else "vlm"
            url = node.get_service_url(service_mode)
            if not url:
                logger.warning(f"[FABRIC] Skipping node {node.hostname}: service '{service_mode}' has no valid URL.")
                continue
                
            # Determine if this represents our local workstation host (which can be booted asynchronously)
            is_local_host = node.hostname.lower() in ("localhost", "127.0.0.1", "workstation-host") or node.hostname.lower() == socket.gethostname().lower()
            
            # For local loopback, force 127.0.0.1 as the host to support standard local Ollama/VLM daemon bindings
            if is_local_host:
                port = node.services.get(service_mode, 11434)
                url = f"http://127.0.0.1:{port}"
            
            srv_name = "Local Workstation" if is_local_host else f"Remote {node.hostname}"
            srv_batch_size = min(node.max_batch_size, args.batch_size) if node.max_batch_size else args.batch_size
            
            srv = wsl_client.VLMServerConnection(
                name=srv_name,
                base_url=url,
                mode=service_mode,
                model_name=args.model,
                batch_size=srv_batch_size
            )
            
            # Double check if remote servers are alive and listening
            if is_local_host:
                logger.info(f"[FABRIC] Target '{srv.name}' is local host. Registering connection dynamically.")
                active_servers.append(srv)
            else:
                alive = srv.is_alive()
                if node.is_online and alive:
                    logger.info(f"[FABRIC] Target '{srv.name}' is online and verified alive. Registering connection.")
                    active_servers.append(srv)
                else:
                    logger.info(f"[FABRIC] Skipped target '{srv.name}' (Online: {node.is_online}, Port Handshake Alive: {alive})")

        logger.info(f"[FABRIC] Fabric setup completed. Targeting {len(active_servers)} active VLM server(s):")
        for srv in active_servers:
            logger.info(f"[FABRIC]   -> {srv.name} @ {srv.base_url} (Model: {srv.model_name}, Batch Size: {srv.batch_size})")

                
    except Exception as e:
        logger.warning(f"[FABRIC] Failed to initialize FabricManager or query PostgreSQL registry: {e}. Falling back to standard local settings.")
        
    # Ultimate local loopback fallback if registry is completely empty or failed
    if not active_servers:
        logger.info("[FABRIC] Falling back to standard local loopback VLM server configuration.")
        local_server = wsl_client.VLMServerConnection(
            "Local Workstation", 
