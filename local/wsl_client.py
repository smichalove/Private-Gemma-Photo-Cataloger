...
class VLMServerConnection:
    ...
    def __init__(self, name: str, base_url: str, mode: str = "vlm", model_name: str = "", batch_size: int = 2) -> None:
        ...
        Args:
            ...
            mode: Server type mode ('vlm' or 'ollama').
            model_name: Target model name (only used if mode='ollama').
            batch_size: Default batch size for this server.
        """
        self.name = name
        ...
        if self.mode == "ollama":
            self.describe_url = f"{self.base_url}/api/chat"
    ...
    def query(
        ...
        if self.mode == "ollama":
            ...
            for img_b64 in images_base64:
                # Implement explicit system and user role separation to comply with prompt safety rules
                payload = {
                    ...
                    "messages": [
                        ...
                        {
                            "role": "user",
                            "content": "Analyze this image according to the archival schema.",
                            "images": [img_b64]
                        }
                    ],
                    "stream": False,
                    "think": False,  # Suppress internal thinking/reasoning blocks to prevent output truncation
                    "options": {
                ...
                if response.status_code == 200:
                    ...
                    responses.append(message_obj.get("content", ""))
                else:
                    raise requests.RequestException(
                        f"Ollama server {self.name} returned HTTP error {response.status_code}: {response.text}"
                    )
            return responses
        else:
